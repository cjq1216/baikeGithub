...
# 3. 启动(APP_ENV 默认 'dev',自动 load .env.dev 的 localhost 默认值)
...
`app/__init__.py:1-25` 顶部用 `python-dotenv` 加载 `.env.{APP_ENV}`(默认 `dev`)。
- `APP_ENV=dev|prod|staging` 决定加载哪个文件
- `APP_ENV=test` 是 pytest 模式(conftest 预 setdefault DB_*,app 跳过 load_dotenv)
- shell env / `docker --env-file` 注入的变量**优先于**文件值(`override=False` 默认行为)
- 生产:必传 5 个 `DB_*` + `FLASK_SECRET`,缺失时 `app/__init__.py:97-113` 抛清晰错误
...
- **Python 2 代码**: `reload(sys)`、`mysql-python`、`sys.setdefaultencoding` 都需要在迁移到 Python 3 时移除(改为 `PyMySQL` + `pymysql.install_as_MySQLdb()` shim、去掉 reload 块)。
